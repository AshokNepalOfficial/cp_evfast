          ...
          <nav className="hidden lg:flex items-center gap-0">
            {navLinks.map((link) => (
              ...
              >
                <Link
                  href={link.href}
                  className="flex items-center px-4 py-2 text-sm font-medium text-gray-700 hover:text-emerald-600 transition-colors relative after:absolute after:bottom-0 after:left-4 after:right-4 after:h-[2px] after:bg-emerald-600 after:scale-x-0 hover:after:scale-x-100 after:transition-transform after:duration-200"
                >
                  {link.label}
                </Link>
                  ...
                  <div className="absolute top-full left-0 mt-0 w-64 bg-white shadow-lg border border-gray-100 py-2 z-50">
                    {link.children.map((child) => (
                      <Link
                        key={child.label}
                        href={child.href}
                        className="block px-5 py-2.5 text-sm text-gray-600 hover:text-emerald-600 hover:bg-emerald-50 transition-colors"
                      >
                        {child.label}
                      </Link>
                    ))}
      ...
      {isOpen && (
        <div className="lg:hidden bg-white border-t border-gray-100 shadow-xl">
          <div className="max-w-7xl mx-auto px-4 py-3 space-y-0">
            {navLinks.map((link) => (
              <div key={link.label}>
                <div className="flex items-center justify-between">
                  <Link
                    href={link.href}
                    className="flex-1 block px-4 py-3 text-sm font-medium text-gray-800 hover:text-emerald-600 transition-colors"
                  ...
                  {link.children && (
                    <button
                      className="px-3 py-3 text-gray-400 hover:text-emerald-600"
                      onClick={() =>
                        setMobileExpanded(
                  ...
                  <div className="ml-4 border-l-2 border-gray-100 pl-4 space-y-0 pb-2">
                    {link.children.map((child) => (
                      <Link
                        key={child.label}
                        href={child.href}
                        className="block py-2 text-sm text-gray-500 hover:text-emerald-600 transition-colors"
                        onClick={() => {
                          setIsOpen(false);
            ...
            <Link
              href="/contact-us"
              className="block mt-3 bg-emerald-600 text-white text-center text-sm font-semibold px-5 py-3 rounded-lg"
              onClick={() => setIsOpen(false)}
            >
              Get a Quote
            </Link>

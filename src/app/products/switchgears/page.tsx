...
export default function SwitchgearsPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100">
      {/* Header */}
      <div className="relative overflow-hidden pt-16 pb-12 bg-gradient-to-r from-emerald-600 to-green-600">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
          {sortedProducts.map((product, index) => (
            <Link
              key={product.id}
              href={product.link}
              className="group bg-white rounded-xl overflow-hidden border border-slate-200 hover:border-emerald-400 shadow-sm hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1"
              style={{
                animationDelay: `${index * 50}ms`,
                animation: 'fadeInUp 0.6s ease-out forwards'
              }}
            >
              {/* Product Image - fixed aspect ratio with object-contain for full visibility */}
              <div className="relative aspect-square w-full bg-slate-100 overflow-hidden">
                ...
                />
                <div className="absolute top-4 right-4 bg-emerald-600 text-white px-3 py-1 rounded-full text-xs font-semibold flex items-center gap-1 shadow-md">
              ...
              {/* Product Info */}
              <div className="p-5">
                <h3 className="text-base font-bold text-slate-900 line-clamp-2 group-hover:text-emerald-600 transition-colors mb-2">
                  {product.title}
                </h3>
                  ...
                  <div className="mb-3 space-y-1">
                    {product.specs.slice(0, 2).map((spec, idx) => (
                      <p key={idx} className="text-xs text-slate-600 flex items-center gap-1">
                        <span className="text-emerald-500">✓</span> {spec}
                      </p>
                    ))}
                  </div>
                )}
                ...
                {/* Action */}
                <div className="flex items-center text-emerald-600 font-medium text-xs group-hover:gap-2 transition-all duration-300">
